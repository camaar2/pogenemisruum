...
- • Õige tulemüürireeglite komplekt tagab, et ainult turvaline ja vajalik liiklus pääseb läbi, hoides pahatahtlikud katsed eemal ja samal ajal lubades tähtsatel teenustel töödata.`  
  },
  {
    ...
    content: `
- **HTTP (HyperText Transfer Protocol)** on protokoll, mida brauser ja veebiserver kasutavad andmete vahetamiseks (veebilehe laadimiseks). HTTP-liiklus liigub tavaliselt **port 80-l** (nagu tänav, mille kaudu sõidavad tavalised autod, mis ei saa saladusi kaasas kanda).  
...
- • Turvalisem variant on kasutada **port 587-t** või **sadamat 465-t**, mis mõlemad toetavad **TLS-krüpteerimist** – see on nagu suletud ümbrik e-kirja jaoks, nii et ainult õiged osapooled saavad selle lugeda.  
...
- • DNS on nagu interneti telefoniraamat: kui soovid külastada veebilehte, küsib sinu arvuti DNS-serverilt, milline IP koosneb domeeniga "example.com". DNS vastab IP-aadressiga, millele brauser seejärel ühenduse loob.  
- • Kui DNS-server on rünnatud või valeinfo sisestatakse (nt **DNS-poisok või DNS-spoofing**), võid sa sattuda petturi lehele, kuigi arvasid, et läksid “õigele” veebisaidile.  
...
- • Kujuta ette, et SSH on nagu telefonikõne, kus räägid teistega salajases kanalis, mida keegi teine ei kuule. Telnet on nagu rääkida telefoni ilma kaitsekotikuta – kogu jutt võib avaneda kolmandatele isikutele.  
- • SSH kasutamine on kõige turvalisem viis kaugjuurdepääsuks serveritesse või võrguseadmetesse, sest kogu liiklus on krüpteeritud. Telnetit ei tohiks kunagi kasutada avalikus või ebaturvalises võrgus, sest ründaja saab kõik sõnumid pealt kuulata.`  
  },
  {
    id: 6,
    title: "Mis on FTP?",
    content: `
- **FTP (File Transfer Protocol)** on vanakooli failiedastusprotokoll, mis töötab tavaliselt **port 21-l** (juhtimisprotokoll) ja **port 20-l** (andmeedastus).  
- • FTP edastab paroolid, failinimed ja failide sisu **ilma krüpteerimata** – see on nagu saata postkaart ilma ümbrikuta: igaüks, kes võrguliikluse pealt kuulab, näeb, mida sa saadad.  
...
- • Kui sinu serveris on vaja faile edastada (näiteks veebifailid või aruanded), ära kasuta FTP-d, sest keegi võib faile pealt kuulata või muudele pahatahtlikele allikatele edastada. Kasuta alati turvalist versiooni, et andmed püsiksid konfidentsiaalsed.`  
  }
];
